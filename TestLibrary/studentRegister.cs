...
namespace Layout
{
    ...
    {
        string firstName, lastName, email, course, section, phoneNumber, password;
        Boolean validFirstName, validLastName, validEmail, validSection, validPassword, validPhoneNumber, validConfirmPassword;

        string emailPattern = "^[a-zA-Z]+\\.[0-9]{6}@laspinas\\.sti\\.edu\\.ph$";
        string sectionPattern = "^[0-9]{3}$";
        ...
        {
            if (Regex.IsMatch(txtSection.Text, sectionPattern))
            {
                lblSection.ForeColor = Color.Green;
                validSection = true;
            }
            else if (txtSection.Text.Equals(""))
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (validFirstName & validLastName & validEmail & validSection & validPassword & validPhoneNumber &
                validConfirmPassword)
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkEmailExistsQuery = "SELECT * FROM [dbo].[Students] WHERE email = @email";
                    ...
                    */
                    ...
                    string stmt = "SELECT COUNT(*) FROM [dbo].[Students]";
                    int count = 0;
                    using (SqlCommand cmdCount = new SqlCommand(stmt, connection))
                    {
                        ...
                        String st = "INSERT INTO [dbo].[Students](Id,firstName,lastName, email, course, section, phoneNumber, password) values " +
                            "(@Id, @firstName, @lastName, @email, @course, @section, @phoneNumber, @password)";
                        ...
                        cmd.Parameters.AddWithValue("@section", Int32.Parse(txtSection.Text));
